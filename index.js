...
function formatarPorcentagem(id) {
    ...
    const valorFormatado = new Intl.NumberFormat('pt-BR', {
        ...
        minimumFractionDigits: 2,
        maximumFractionDigits: 2
    }).format(apenasNumeros / 100); // estou dividindo o valor por 100 para formatar como porcentagem
    document.getElementById(id).value = valorFormatado;
}

function formatarValor(id) {
    const apenasNumeros = document.getElementById(id).value.replace(/[^0-9.,]/g, '');
    const valorFormatado = new Intl.NumberFormat('pt-BR', {
        style: 'decimal',
